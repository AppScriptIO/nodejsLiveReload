...
const childProcess = require('child_process'); // import proxyMiddleware from 'http-proxy-middleware'
...
class ServerLivereload extends EventEmitter {
  constructor(debugArguments, entrypoint) {
    ...
    this.entrypoint = entrypoint; // clean up if an error goes unhandled.
  ...
  reload() {
    if (this.node) this.node.kill(); // node = childProcess.fork('babelCompile.entrypoint.js', { cwd: '/app/serverSide', stdio:'inherit', execArgv: debugArguments})

    this.node = childProcess.fork(this.entrypoint.filename, {
      cwd: this.entrypoint.filePath,
      stdio: 'inherit',
      execArgv: this.debugArguments
    });
